...
When a mw.\* helper is deprecated in favor of a native equivalent (e.g.
a URL-parsing wrapper deprecated in favor of the native `URL` class),
verify the native replacement preserves the same failure mode before
swapping — e.g. both must throw synchronously on invalid input — so
calling code’s error handling does not need to change. After swapping,
grep the module’s ResourceLoader dependency list for the now-unnecessary
mw.\* module and remove it only if this was genuinely the last usage;
state in the commit message that you checked.
